...
# Modified by Alfredo Schez.-R., 2019-09-07
...
AC_DEFUN([LB_CHECK_GMP],
  ...
  GMP_HOME_PATH=
  AC_ARG_WITH(gmp,
    AS_HELP_STRING([--with-gmp={DIR|yes|no}],
  ...
  if test "x$GMP_HOME_PATH" = "x"; then
    ...
    for GMP_HOME in ${GMP_HOME_PATH}; do
      if test -r "$GMP_HOME/include/gmp.h" -o -r "$GMP_HOME/include/gmpxx.h"; then
        gmp_found="yes"
